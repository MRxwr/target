...
<div class="pull-left">
<h6 class="panel-title txt-dark"><?php echo direction("List of Invoices", "قائمة الفواتير") . " [ {$order[0]["fName"]} {$order[0]["mName"]} {$order[0]["lName"]} ]" ?></h6>
</div>
...
<div class="table-responsive">
	...
	<table class="table display responsive product-overview mb-30" id="myTable">
		...
		<tbody>
		<?php 
		if( $orders = selectDB("orders","`fName` LIKE '{$order[0]["fName"]}' AND `mName` LIKE '{$order[0]["mName"]}' AND `lName` LIKE '{$order[0]["lName"]}' AND `phone` = '{$order[0]["phone"]}' ORDER BY `date` DESC") ){
